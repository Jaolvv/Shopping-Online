...
(function($) {
	...
	function tree_component () {
		...
		return {
			...
			//重新载入此节点,增加/修改/刷新操作当前节点，删除操作上级节点
			refreshEx:function(dataJson) {
				var obj =   dataJson['node'];
				var _this = this;
				$.ajax({
					type		: this.settings.data.method,
					url			: dataJson.url, 
					data		: dataJson.parameter, 
					dataType	: "json",
					success		: function (data, textStatus) {
						var data = _this.settings.callback.onJSONdata.call(null, data, _this);
						...
						obj.before(str);
						obj.prev("li:last-child").addClass("last").end().find("li:has(ul)").not(".open").addClass("closed");
						obj.prev().find("li").not(".open").not(".closed").addClass("leaf");
						
						_this.open_branch.apply(_this, [obj.prev()]);
						_this.settings.callback.onSuccess(obj.prev());
					...
					if(this.settings.data.json) {
						...
						$.ajax({
							...
							success		: function (data) {
								...
								_this.settings.callback.onSuccess(null);
							},
					...
					if(this.settings.data.type == "xml_flat" || this.settings.data.type == "xml_nested") {
						...
						$.ajax({
							...
							success		: function (data, textStatus) {
								...
								_this.settings.callback.onSuccess(obj);
							},
