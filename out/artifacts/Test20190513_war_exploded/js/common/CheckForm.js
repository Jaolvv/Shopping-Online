	...
	for (i=0;i<aa.length;i++)
	{
		...
		if(aa[i].getAttribute('check_str')!=""&&aa[i].getAttribute('check_str')!=null)
		{
			...
			if(obj.value.length==0)
			{
				...
				{
					if(obj.getAttribute('check_type')=="onlyAlert") {
						alert(obj.getAttribute('check_str'),"<br>");
					}
					else {
			...
			if(obj.getAttribute('check_type')=="phoneNo")
			{
				if(!checkPhoneNo(obj))
				{
					alert("您输入的【" + obj.getAttribute('check_str')+"】不是合法的电话号码,正确的格式为：\r\n本地手机号码或固定电话号码、长途区号+外地电话\r\n如：65661238、02365661238、023-65661238","<br>");
					break;
				}
			}

			if(obj.getAttribute('check_type')=="mobileNo")
			{
				if(!checkMobileNo(obj))
				{
					alert("您输入的【" + obj.getAttribute('check_str')+"】不是合法的手机号码","<br>");
					break;
				}
			}
			
			//验证ip
			if(obj.getAttribute('check_type')=="ip")
			{
				if(!checkIp(obj))
				{
					alert("您输入的" + obj.getAttribute('check_str')+"不是合法的IP地址","<br>");
					break;
				}
			}
			
			//对定长的数字处理：例如邮编
			if(/^specNumber/.test(obj.getAttribute('check_type'))) {
				tempArr = checkSpecNumber(obj);
				if(!tempArr[0])
				{
					alert(tempArr[1]);
					break;
...
function checkPhoneNo(obj)
{
	...
	var valid = false;
	valid = /^[1-9][0-9]*$/.test(obj.value);
	//不合法就继续测试，合法就不测试了
	if(!valid) {
		valid = /^0[0-9]{2,5}[-]?([1-9][0-9]*)$/.test(obj.value);
	}
...
function checkMobileNo(obj)
{
	...
	return  /^0?1[0-9]{10}$/.test(obj.value);
}
...
function checkIp(obj)
{
	...
	return obj.value == 'localhost' || /^(?:(?:25[0-5]|2[0-4]\d|[01]?\d?\d)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d?\d)$/.test(obj.value);
}
...
function checkSpecNumber(obj){
	...
	{
		tempArr[0]=false;
		tempArr[1]="不是合法的数字，请重新输入" + obj.getAttribute('check_str')+"";
		return tempArr;
	}
